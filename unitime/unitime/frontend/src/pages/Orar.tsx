...
interface Orar { id: number; titlu: string; data: string; ora: string }

export default function Orar() {
    const [items, setItems] = useState<Orar[]>([])
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                    {items.map(o => (
                        <li key={o.id} style={{
                            ...
                            <Link to={`/orar/${o.id}`} style={{
                                color: 'white',
                                textDecoration: 'none',
                                fontSize: '1rem'
                            }}>
                                {o.titlu} — {o.data} @{o.ora}
                            </Link>
                        </li>
                    ))}
                </ul>
